	...
	for (pair<string, vector<pair<string, string>>> pa : matrix)
	{
		...
		if (constraints.at(row) == "N")
		{
			...
			for (pair<string, string> pa1 : contents)
			{
				...
				if (variable_type.at(pa1.first) == "Real")
					isreal = true;
			}
			if (isreal)
			{
				constraints_rhs.at(row) = "(+ " + to_real(optresult) + " " + to_real(constraints_rhs.at(row)) + ")";
			}
			else
			{
				constraints_rhs.at(row) = "(+ " + optresult + " " + constraints_rhs.at(row) + ")";
			}
			if (isreal)
			{
