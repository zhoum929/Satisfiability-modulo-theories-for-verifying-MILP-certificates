...
RemoteExecutionManager *remoteExecutionManager = new RemoteExecutionManager();
...
void check() {
    remoteExecutionManager->dispatch(filename + "_vipr.smt2", line_num);
    ...
    if((first_faulty_dispatch = remoteExecutionManager->collect_ready_dispatches()) != nullptr) {
        cerr << "Error in VIPR line " << first_faulty_dispatch->line;
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    // cout <<"Do you want to specify the number of derived constraints to be checked in each block? [Y/N]"<<endl;
    s = argv[2];
    cout << "Specified the number of derived constraints to be checked in each block? [Y/N]" << s << endl;
    if(s=="Y"){
    ...
    while (true)
    {
        ...
        if (words_of_line[0] == "VAR")
        {
            ...
            if((first_faulty_dispatch = remoteExecutionManager->clear_dispatches()) != nullptr) {
                cerr << "Error in VIPR line " << first_faulty_dispatch->line;
                exit(1);
            }

            delete remoteExecutionManager;
            ...
            cout << filename << "," << block_size << "," << elapsed.count() * 1e-9 << ",(Result)" << endl;
            return 0;
        }
    }
